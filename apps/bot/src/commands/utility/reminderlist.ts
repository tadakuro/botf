...
const command: Command = {
  ...
  async execute(interaction) {
    ...
    const r = await Model.find({ userId: interaction.user.id, sent: false }).sort({ remindAt:1 }).exec();
    if (!r || !r.length) return interaction.reply({ embeds: [infoEmbed('No reminders.')], ephemeral: true });
