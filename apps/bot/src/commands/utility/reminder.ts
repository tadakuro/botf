...
const command: Command = {
  ...
  async execute(interaction) {
    ...
    await ReminderModel.create({ userId: interaction.user.id, guildId: interaction.guild?.id, reason: interaction.options.getString('reason',true), remindAt: new Date(Date.now()+ms) });
